...
class PDFService {
	...
	generateNotes() {
		...
		for (i = 0; i < this.data.notes.length; i++) {
			...
			this.doc
				...
				.font('Helvetica-Bold')
				.text(
					`Create: ${year} ${mouth} ${getDate}`,
					310,
					positions[i] + (rowHeight - 30),
					{
						align: 'right',
						characterSpacing: this.lineHeight - 0.2,
					}
				);
			this.doc.moveDown();
	...
	generateQuestionnaire(questionnaire) {
		...
		for (index = 0; index < questionnaire.length; index++) {
			const { type, flagged, question, answers } = questionnaire[
				index
			];
			...
			this.doc
				...
				.text(`Answer: ${answer}`, 300, positions[index], {
					align: 'justify',
					width: 220,
				});
			this.doc
				...
				.lineTo(290, positions[index + 1] - 1)
				.stroke();
			this.generateHr(positions[index + 1] - 1); //!Line
			Height = positions[index + 1] - 1;
		}
		return Height;
	}
